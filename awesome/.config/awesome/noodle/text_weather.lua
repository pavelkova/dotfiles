...
local key = "8ae0bb9dc8c1abc57ba0bfde3d8a5b25"
local city_id = "4580549"
local units = "metric"
local symbol = "°"
...
local function update_widget(icon_code, weather_details)
  ...
  if string.find(icon_code, "01d") then
    weather_icon.markup = helpers.colorize_text(sun_icon, beautiful.xcolor15)
  elseif string.find(icon_code, "01n") then
    weather_icon.markup = helpers.colorize_text(moon_icon, beautiful.xcolor15)
  elseif string.find(icon_code, "02d") then
    weather_icon.markup = helpers.colorize_text(dcloud_icon, beautiful.xcolor15)
  elseif string.find(icon_code, "02n") then
    weather_icon.markup = helpers.colorize_text(ncloud_icon, beautiful.xcolor15)
  elseif string.find(icon_code, "03") or string.find(icon_code, "04") then
    weather_icon.markup = helpers.colorize_text(cloud_icon, beautiful.xcolor15)
  elseif string.find(icon_code, "09") or string.find(icon_code, "10") then
    weather_icon.markup = helpers.colorize_text(rain_icon, beautiful.xcolor15)
  elseif string.find(icon_code, "11") then
    weather_icon.markup = helpers.colorize_text(storm_icon, beautiful.xcolor15)
  elseif string.find(icon_code, "13") then
    weather_icon.markup = helpers.colorize_text(snow_icon, beautiful.xcolor15)
  elseif string.find(icon_code, "50") or string.find(icon_code, "40") then
    weather_icon.markup = helpers.colorize_text(mist_icon, beautiful.xcolor15)
  else
    weather_icon.markup = helpers.colorize_text(whatever_icon, beautiful.xcolor15)
  end
  ...
  weather_text.markup = string.upper(weather_details)
end

local weather_details_script = [[
  bash -c '
   weather=$(curl -sf "http://api.openweathermap.org/data/2.5/weather?APPID=8ae0bb9dc8c1abc57ba0bfde3d8a5b25&id=4580549&units=metric")
